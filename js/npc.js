...
const NPC_DEFS = [
    ...
    },
    {
        ...
        dialogue: {
            default: ["*sobbing dramatically* THREE DAYS! THREE DAYS WITHOUT TACOS! The kingdom is CRUMBLING!",
                      ...
                      "*contentedly munching tacos* Ah, a champion arrives! I have MANY important tasks for you.",
                      "I am your King. I am also your greatest quest giver. These things are related."],
        ...
        talkCallback: ['kings_taco_emergency:return_king', 'royal_sock_search:return_socks',
                       'kings_goblin_census:report_census', 'kings_royal_inspection:report_inspect',
                       'kings_tournament:claim_prize'],
    ...
    /* More Castle Guards */
    { id: 'guard_gate2',    name: 'Guard Tom',     x: -5,  z: -86, skinHex: 0xFFD5B0, clothHex: 0x606070, hairHex: 0x5D3A1A, hairStyle: 'short',
      dialogue: { default: ["Nothing gets past me! ...Except that one goblin last Thursday. We don't mention that.","Halt! Actually, you look friendly. Carry on."] } },
    { id: 'guard_west',     name: 'Guard Petra',   x: -30, z: -100, skinHex: 0xFFE8D6, clothHex: 0x707080, hairHex: 0xC0392B, hairStyle: 'braid',
      dialogue: { default: ["West wall is secure. The east wall... we try not to look at it too hard.","I've been on guard duty 12 hours. Ask me how I feel. Actually don't."] } },
    { id: 'guard_east',     name: 'Guard Bruno',   x: 30,  z: -100, skinHex: 0xD4956A, clothHex: 0x606070, hairHex: 0x1A1A1A, hairStyle: 'short',
      dialogue: { default: ["East wall. Very exciting. Nothing ever happens here. Please something happen.","*yawning* Fourth watch. My boots are killing me. What brings you to the east wall?"] } },
    { id: 'guard_court1',   name: 'Guard Mathis',  x: 10,  z: -110, skinHex: 0xFFD5B0, clothHex: 0x707070, hairHex: 0x9E9E9E, hairStyle: 'short',
      dialogue: { default: ["Courtyard duty. The King wanted guards who \"look impressive.\" I have no idea if I do.","Stand straight! Or don't. The King is currently distracted by tacos."] } },
    { id: 'guard_court2',   name: 'Guard Elsa',    x: -10, z: -110, skinHex: 0xFFD5B0, clothHex: 0x707080, hairHex: 0xF5D03A, hairStyle: 'short',
      dialogue: { default: ["Courtyard patrol, hour seven. I've counted all the cobblestones. There are 847.","Training is on Wednesdays. Today is Wednesday. I missed training. To do guard duty."] } },
    { id: 'guard_throne',   name: 'Guard Roderick', x: -8, z: -130, skinHex: 0xC49A6C, clothHex: 0x555555, hairHex: 0x1A1A1A, hairStyle: 'short',
      dialogue: { default: ["Throne room guard. High honor. Also high smell of taco wafting from within.","The King is... emotional today. About tacos. As always."] } },
    { id: 'guard_throne2',  name: 'Guard Isolde',  x:  8,  z: -130, skinHex: 0xFFE8D6, clothHex: 0x606060, hairHex: 0x5D3A1A, hairStyle: 'braid',
      dialogue: { default: ["I've been trained in 12 forms of combat. I now guard a man eating tacos.","Duty is duty. The throne room smells wonderful today though, I'll admit."] } },
    { id: 'guard_wall_n1',  name: 'Guard Finn',    x: -20, z: -152, skinHex: 0xFFD5B0, clothHex: 0x707070, hairHex: 0x9E9E9E, hairStyle: 'bald',
      dialogue: { default: ["North wall. The view of the mountains is nice. That's the only nice thing I can say."] } },
    { id: 'guard_wall_n2',  name: 'Guard Petra II',x: 20,  z: -152, skinHex: 0xD4956A, clothHex: 0x606070, hairHex: 0x1A1A1A, hairStyle: 'short',
      dialogue: { default: ["I patrol this stretch 40 times per shift. I know every crack in this wall personally."] } },
    { id: 'guard_tower_sw', name: 'Guard Brennan', x: -30, z: -150, skinHex: 0xFFD5B0, clothHex: 0x707080, hairHex: 0x5D3A1A, hairStyle: 'short',
      dialogue: { default: ["Southwest tower. Best view in the castle. I refuse to complain about this post."] } },
    { id: 'guard_tower_se', name: 'Guard Nadia',   x: 30,  z: -150, skinHex: 0xFFE8D6, clothHex: 0x606060, hairHex: 0xF5D03A, hairStyle: 'short',
      dialogue: { default: ["Southeast tower. I wave to Brennan sometimes. He never waves back. He's very professional."] } },
    { id: 'guard_patrol1',  name: 'Guard Sven',    x: 0,   z: -95,  skinHex: 0xFFD5B0, clothHex: 0x707070, hairHex: 0x9E9E9E, hairStyle: 'short',
      dialogue: { default: ["On patrol! Just walked from gate to keep. Now walking back. Excellent progress."] } },

    /* Village Expansion NPCs */
    { id: 'tanner_greta',   name: 'Tanner Greta',  x: 32,  z: 27,  skinHex: 0xD4956A, clothHex: 0x5C3A1A, hairHex: 0xC0392B, hairStyle: 'braid',
      dialogue: { default: ["I tan leather! It's smelly work but someone has to do it. Preferably me, as I enjoy it.","Best hides in the region. Some might even say they're worth dying for. The goblins do, apparently."] } },
    { id: 'herbalist_vera',  name: 'Herbalist Vera', x: -28, z: 22, skinHex: 0xFFE8D6, clothHex: 0x2E8B57, hairHex: 0xF5F5F5, hairStyle: 'long',
      dialogue: { default: ["Every plant has a purpose! Even the ones that smell terrible. ESPECIALLY those.","I sell remedies, salves, and one tea that makes you see colors. I don't recommend that one."] } },
    { id: 'weaver_magnus',   name: 'Weaver Magnus', x: 30,  z: -20, skinHex: 0xFFD5B0, clothHex: 0x4B0082, hairHex: 0x9E9E9E, hairStyle: 'short',
      dialogue: { default: ["Finest cloth in the kingdom! I've been weaving for forty years. My back is fine. It is not fine.","Would you like a cape? I'm working on one. Red, very dramatic. Good for questing."] } },
    { id: 'village_smith2',  name: 'Smith Owen',    x: 40,  z: -13, skinHex: 0xD4956A, clothHex: 0x222222, hairHex: 0x1A1A1A, hairStyle: 'bald',
      dialogue: { default: ["*clang* *clang* WHAT? Oh. Sorry. Focused.","I make the best weapons. Hank in the old forge makes fine work too, but don't tell him I said that."] } },
    { id: 'market_vendor',   name: 'Vendor Pip',    x: -13, z: 40,  skinHex: 0xC49A6C, clothHex: 0xDAA520, hairHex: 0x5D3A1A, hairStyle: 'short',
      dialogue: { default: ["FOOD! FRESH FOOD! Bread, meat, cheese, and something that MIGHT be a vegetable!","Everything must go! Well, not must. But I'd appreciate it. Overhead costs are real."] } },
    { id: 'market_baker2',   name: 'Baker Fen',     x: -18, z: 38,  skinHex: 0xFFE8D6, clothHex: 0xFFB347, hairHex: 0x5D3A1A, hairStyle: 'short',
      dialogue: { default: ["Pies! Stews! Bread! The bread is slightly cursed but in a GOOD way.","I bake, therefore I am. Slightly covered in flour. Both are fine."] } },
